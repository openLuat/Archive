...
ifeq ($(strip $(AM_DISTCC_SUPPORT)), TRUE)
export CC := distcc $(CROSS)gcc
else
export CC := $(CROSS)gcc
endif
endif
...
ifeq ($(strip $(AM_SUBPROJ_SUPPORT)), TRUE)
	...
	find $(DES_CFP_FILE_DIR) -name "*.cfp" -delete ;
...
ifneq "${AM_PLT_LOD_FILE}" ""
	@${ECHO}
	@${ECHO} "LODTOBIN $(LOD_TO_BIN) ${LOD_FILE} -0"
...
ifeq ($(strip $(AM_CONFIG_SUPPORT)), TRUE)
	...
	@${ECHO} "Creat map Zip..."
	cd $(BINARY_PATH) && 7z a $(ZIP_BASENAME)_map.zip -r ${ZIP_BASENAME}_map;
	@${ECHO} "Creat map Zip     Creat map Zip Sucessful"
	@${ECHO}
